            ...
            for file_name in csv_files:
                ...
                with z.open(file_name) as f:
                    for chunk in pd.read_csv(f, index_col=None, names=base_self.__fts_names, header=0, chunksize=10000, low_memory=False):
                        ...
                        chunk[base_self.__target_variable] = os.path.basename(file_name)[2:-4]
    ...
    if load_type=="raw":
      ...
      if os.path.exists(data_dir) == True:
        ...
        if is_zipfile(data_file):
          ...
          print(f"Skipping extraction to save disk space. ZIP available at: {data_file}")
        else:
  ...
  def Load_Data(base_self, datadir = os.getcwd(), device_name="all", load_type="raw", limit_cnt=sys.maxsize, frac = None):
    ...
    if load_type=="raw":
      ...
      if os.path.exists(datapath) or os.path.exists(datapath + ".zip"):
        ...
        # Handle zip paths differently for stream processing
        if not os.path.exists(datapath) and os.path.exists(datapath + ".zip"):
             datapath = datapath + ".zip"
