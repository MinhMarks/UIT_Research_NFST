...
for metric in ['local', 'global', 'cluster']:
    df[f'Avg_{metric}'] = df[[f'CICIoT_{metric}', f'N_BaIoT_{metric}', f'ToNIoT_{metric}', f'IoTID20_{metric}']].mean(axis=1)

df['Total_Mean'] = df[['Avg_local', 'Avg_global', 'Avg_cluster']].mean(axis=1)

# Find best and second best
best_vals = {}
for col in ['CICIoT_local', 'N_BaIoT_local', 'ToNIoT_local', 'IoTID20_local', 'Avg_local',
            'CICIoT_global', 'N_BaIoT_global', 'ToNIoT_global', 'IoTID20_global', 'Avg_global',
            'CICIoT_cluster', 'N_BaIoT_cluster', 'ToNIoT_cluster', 'IoTID20_cluster', 'Avg_cluster',
            'Total_Mean']:
...
for orig_name, latex_name in models.items():
    ...
    for metric in ['local', 'global', 'cluster']:
        for ds in ['CICIoT', 'N_BaIoT', 'ToNIoT', 'IoTID20', 'Avg']:
