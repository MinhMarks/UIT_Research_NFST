...
def calculate_NPD(X, y, epsilon=1e-6):
    ...
    tracemalloc.stop()
    print(f"--- Peak Memory for Learn API: {peak_mem / 10**6:.2f} MB ---")
    print(f"Size of null_point_X: {sys.getsizeof(null_point_X) / 10**6:.2f} MB")
    print(f"Size of null_point_X_test: {sys.getsizeof(null_point_X_test) / 10**6:.2f} MB")
    print("...............................Timing Model................................")
...
def Model_evaluating(y_true, y_predict, y_scores):
    ...
    # Lật nhãn dữ liệu (0 thành 1, đánh giá tập trung lớp thiểu số Normal)
    y_true_flipped = 1 - y_true
    # Lấy xác suất của lớp 0 (nay trở thành positive)
    y_prob = y_scores[:, 0]
    ...
    mcc = matthews_corrcoef(y_true_flipped, y_predict_optimal)
    f1 = f1_score(y_true_flipped, y_predict_optimal)
    ppv = precision_score(y_true_flipped, y_predict_optimal, zero_division=0)
    recall = recall_score(y_true_flipped, y_predict_optimal, zero_division=0)
    accuracy = accuracy_score(y_true_flipped, y_predict_optimal)
    auc = roc_auc_score(y_true_flipped, y_prob)
    aucpr = average_precision_score(y_true_flipped, y_prob)
...
def function(df1,df2, scaler, noise):
    ...
    for ncluster in range (1,301,3):
        ...
        result = [scaler] + [ncluster] + [noise] + v + [training_time, inference_time, peak_train / 10**6]
