...
def BruteForce_Threshold(y_true, y_prob, minth=0.0, maxth=1.0, num_thresholds=1000):
    ...
    for threshold in thresholds:
        ...
        acc = accuracy_score(y_true_flipped, y_pred)
        f1 = f1_score(y_true_flipped, y_pred)
        mcc = matthews_corrcoef(y_true_flipped, y_pred)
        auc_roc = roc_auc_score(y_true_flipped, y_prob[:, 0])
        auc_pr = average_precision_score(y_true_flipped, y_prob[:, 0])
